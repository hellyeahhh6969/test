...
spawn(function()
    ...
    local function createGUI(ply)
        ...
        mainPanel.Size = UDim2.new(0, 280, 0, 160)
        ...
        local function createVisualSlider(parent, yPos, labelText, icon, color, minVal, maxVal, defaultValue, callback)
            ...
            sliderBtn.Position = UDim2.new(barFill.Size.X.Scale - 0.025, 0, -4, 0)
            ...
            shadow.Parent = sliderBtn
            shadow.Size = UDim2.new(1, 0, 1, 0)
            shadow.Position = UDim2.new(0, 0, 0.2, 0)
            ...
            sliderBtn.MouseButton1Down:Connect(function()
                ...
                local dragConn = game:GetService("UserInputService").InputChanged:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseMovement and dragging then
                        ...
                        sliderBtn.Position = UDim2.new(percent - 0.025, 0, -4, 0)
        ...
        createVisualSlider(mainPanel, 35, "Boob Size", "💗", Color3.new(1, 0.3, 0.5), 0.3, 1.5, boobScale, function(val)
            boobScale = val
        ...
        createVisualSlider(mainPanel, 90, "Ass Size", "🍑", Color3.new(0.3, 0.5, 1), 0.4, 2.0, assScale, function(val)
            assScale = val
            local torso = character:FindFirstChild("Torso") or character:FindFirstChild("UpperTorso")
            if torso then
                local assBaseOffset = Vector3.new(0.45, -0.7, 0.5)
                ...
                for _, name in ipairs({"RightAss", "LeftAss"}) do
                    ...
                    if part then
                        part.CFrame = torso.CFrame * CFrame.new(
                            (name == "RightAss" and newPos.X or -newPos.X),
                            newPos.Y,
                            newPos.Z
                        )
                        local scaleFactor = val / 1.1
                        part.Size = Vector3.new(1.4 * scaleFactor, 1.0 * scaleFactor, 0.8 * scaleFactor)
                        local mesh = part:FindFirstChildOfClass("SpecialMesh")
                        if mesh then
                            mesh.Scale = Vector3.new(val, val * 0.82, val * 0.82)
                        end
                    end
                end
            end
        ...
        infoPanel.Size = UDim2.new(0, 280, 0, 50)
        infoPanel.Position = UDim2.new(0, 10, 0, 180)
        infoPanel.BackgroundColor3 = Color3.new(0.1, 0.1, 0.15)
        infoPanel.BackgroundTransparency = 0.15
        infoPanel.BorderSizePixel = 1
        infoPanel.BorderColor3 = Color3.new(0.3, 0.3, 0.5)
        
        local infoText = Instance.new("TextLabel")
        infoText.Parent = infoPanel
        infoText.Size = UDim2.new(1, -10, 1, -10)
        infoText.Position = UDim2.new(0, 5, 0, 5)
        infoText.Text = "🔒 Только для владельца | 💗 Грудь: мягкая | 🍑 Ягодицы: упругия"
        infoText.TextColor3 = Color3.new(0.8, 0.8, 0.9)
    ...
    --   ЗАПУСК
    -- ============================================================
    if createAllMeshes(character) then
        createGUI(player)
    end
    ...
    player.CharacterAdded:Connect(function(newChar)
        ...
        character = newChar
        guiCreated = false
        if createAllMeshes(newChar) then
            createGUI(player)
        end
    end)
    
    print("✅ Система активирована!")
    print("🔒 Меши видны ТОЛЬКО владельцу (" .. owner .. ")")
    print("💗 Грудь: мягкая физика (Stiffness=30)")
    print("🍑 Ягодицы: упругая физика (Stiffness=55)")
end
